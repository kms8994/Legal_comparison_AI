...
| 항목 | 기술 |
| :--- | :--- |
| LLM 구조화 | Gemini API, `gemini-2.5-flash-lite` |
| 사용자 질의 구조화 | Gemini API, `gemini-2.5-flash-lite` |
| 임베딩 | Gemini Embeddings API, `gemini-embedding-001` |
| 운영 방식 | LLM은 구조화/검색 질의 생성에 제한 사용 |
...
- MVP에서는 사용자 요청 시 국가법령정보 API를 직접 호출하지 않는다.
- 국가법령정보 API는 로컬 수동 수집 파이프라인에서만 사용한다.
- 수집된 판례는 Supabase DB에 저장하고, 운영 백엔드는 DB만 조회한다.
- 추후 서버 수집 자동화를 붙일 경우 Google Compute Engine VM 고정 외부 IP 또는 별도 백엔드 도메인을 등록한다.
...
- 판결 결과 라벨링
- 판결 이유와 구별 사실 구조화
- 사용자 자연어 입력을 검색용 JSON으로 변환

#### 초기 운영 방침

- 저비용 MVP 기준 모델은 `gemini-2.5-flash-lite`로 시작한다.
- 정확도가 부족한 경우 판례 구조화 작업만 더 상위 Gemini 모델 또는 GPT 계열 모델로 상향한다.
- 사용자 요청 시 LLM은 최종 답변 생성이 아니라 검색 질의 구조화에만 사용한다.
- 선택 판례 간 설명은 우선 DB에 저장된 구조화 필드를 조립해 제공한다.
- 샘플 판례 세트로 품질을 점검한다.
- 평가 항목은 다음과 같다.
  - 판결 결과 라벨 정확도
  - 요약 왜곡 정도
...
flowchart LR
    ...
    API --> QLLM["LLM API<br/>사용자 질의 구조화"]
    API --> QEMB["Embedding API<br/>질의 임베딩"]

    LAW --> COL["수집 파이프라인"]
    COL --> PRE["전처리 파이프라인"]
    PRE --> SLLM["LLM 구조화"]
    SLLM --> EMB["Embedding 생성"]
    EMB --> DB
```
...
| `llm_service` | 판례 구조화, 사용자 질의 구조화, 프롬프트 버전 관리 |
...
flowchart LR
    ...
    B --> C["원문 저장"]
    C --> D["원문 정규화"]
    D --> E["gemini-2.5-flash-lite 구조화 추출"]
    E --> F["gemini-embedding-001 임베딩 생성"]
    F --> G["DB 적재"]
    G --> H["품질 검수"]
```
...
```mermaid
flowchart LR
    A["자연어 입력"] --> B["gemini-2.5-flash-lite 검색 질의 구조화"]
    B --> C["쟁점/사실관계/핵심사실 JSON 생성"]
    C --> D["gemini-embedding-001 질의 임베딩"]
    D --> E["쟁점/사실관계 벡터 검색"]
    E --> F["상위 후보 추출"]
    F --> G["조문/사건유형/결론 라벨 보조 정렬"]
    G --> H["기준 판례와 유사하지만 결론이 다른 판례 반환"]
```
...
LLM은 판례 수집 후 오프라인 구조화 단계에서 사용한다.
...
- 판결 이유 요약
- 결론에 영향을 준 핵심 사실 추출
- 다른 결론을 만들 수 있는 구별 사실 추출

이 단계에서 생성된 결과는 DB에 저장하고, 검색 시 재사용한다.

기본 모델은 저비용 MVP 기준 `gemini-2.5-flash-lite`로 시작한다. 정확도 부족이 확인되면 판례 구조화 작업만 더 상위 Gemini 모델 또는 GPT 계열 모델로 상향한다.
...
- 사용자 자연어 입력을 검색용 JSON으로 변환
- domain, case_type, legal_issue, fact_pattern, key_facts를 추출
- 추출된 검색 질의를 `gemini-embedding-001`로 임베딩
- 최종 비교 설명은 우선 DB에 저장된 구조화 필드를 조립해 제공

사용자 요청 단계에서는 LLM이 법률 판단이나 최종 답변을 직접 생성하지 않는다. LLM은 검색 품질을 높이기 위한 질의 정규화 도구로 한정한다.

### 9.2.1 부족 입력 보완 전략

사용자가 사건을 충분히 설명하지 못하는 경우를 대비해, 서비스가 미리 정의한 필수 검색 슬롯을 기준으로 부족한 정보만 추가 질문한다.

필수 슬롯은 다음과 같다.

| 슬롯 | 목적 |
| :--- | :--- |
| fact_pattern | 사건의 기본 사실관계 파악 |
| parties | 당사자 관계 파악 |
| disputed_action | 문제가 된 행위 또는 처분 파악 |
| claimed_damage_or_result | 손해, 처분, 결과 파악 |
| legal_issue | 사용자가 궁금한 법적 쟁점 파악 |

처리 원칙:

- 사용자 입력 1회당 LLM 호출은 1회로 제한한다.
- LLM은 슬롯 추출만 수행하고 추가 질문 문장을 생성하지 않는다.
- 추가 질문은 코드에 정의된 템플릿에서만 선택한다.
- 필수 슬롯이 2개 이상 부족하면 바로 판례 검색을 하지 않고 추가 입력을 요청한다.
- LLM이 추출한 내용 중 비어 있거나 "불명확"한 값은 검색 근거로 사용하지 않는다.
- 충분한 입력이 들어오면 구조화된 슬롯을 검색 텍스트로 합쳐 DB 검색에 사용한다.

### 9.2.2 데이터 부족 시 수집 요청 큐

입력은 충분하지만 DB에 관련 판례가 부족한 경우, 서비스는 잘못된 판례를 억지로 반환하지 않는다. 대신 `collection_requests` 테이블에 수집 요청을 저장한다.
...
→ collection_requests upsert
→ 관리자/로컬 수집 파이프라인이 pending 요청 처리
```

큐에는 domain, case_type, legal_issue, fact_pattern, key_facts, requested_count, priority를 저장한다. 같은 유형의 요청은 중복 생성하지 않고 requested_count를 증가시킨다.

초기 seed 요청은 법률 초심자가 자주 검색할 가능성이 높은 주제로 미리 넣어둔다.
...
- 일정 횟수 초과 시 실패 상태 저장
- 실패 건은 재처리 큐에 등록
- 사용자 질의 구조화 실패 시 원문 입력을 그대로 임베딩해 검색을 수행한다.
- 구조화 실패가 있어도 검색 결과 자체는 유지한다.
